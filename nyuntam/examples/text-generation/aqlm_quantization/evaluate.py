...
def aqlm_root():
    if "AQLM_ROOT" in os.environ:
        return os.environ["AQLM_ROOT"]
    else:
        print(
            "[WARNING]: env AQLM_ROOT not set. Using default path (text_generation/quantization/aqlm/AQLM, assuming nyuntam as workspace root)."
        )
        return "text_generation/quantization/aqlm/AQLM"


sys.path.append(aqlm_root())
